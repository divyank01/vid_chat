...
app._initializeHTTP = function() {
  ...
  this.get('/:key/id', function(req, res, next) {
    ...
    var randId=self._generateClientId(req.params.key);
    console.log(randId);
    console.log(self._clients);
    res.send(randId);
  });
  ...
  this.get('/:randId/:username/getAll', function(req, res, next) {
    var username=req.params.username;
    var randId=req.params.randId;
    var retVal=[];
    console.log('hitting new post');
    var flag=false;
    for(var k in usernames){
      for(var i in self._clients['peerjs']){
        if(self._clients['peerjs'][k]!=null && k.toString()==i){
    ...
    if(flag){
      ...
      for(var k in usernames){
        count++;
        if(self._clients['peerjs'][k]!=null)
          retVal.push({username:usernames[k],remoteId:k});
      }
      if(count===100){
        usernames={};
      } 
      res.send(JSON.stringify({ type: 'taken',arr:retVal}));
      return;
    }
    var retVal=[];
    usernames[randId]=username;
    var count=0;
    for(var k in usernames){
      count++;
      if(self._clients['peerjs'][k]!=null)
